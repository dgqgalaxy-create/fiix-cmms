...
const labels: Record<WeeklyCategory, string> = {
  ...
  backlog: 'Backlog abierto actual', backlogOverdue: 'Backlog vencido', allCompleted: 'Cierres totales de la semana',
};
...
export function WeeklyWorkOrderProgress({ onOpenOrder }: { onOpenOrder: (id: string) => Promise<void> }) {
  ...
  function excel() {
    ...
    downloadWorkbook(`avance_semanal_${data.week}.xlsx`, [
      ...
      { name: 'Día a día', rows: data.daily.map(d => ({ Día: d.day, Corte: stamp(d.capturedAt), 'Cierres del día': d.closed, 'Cierres acumulados': d.cumulative, 'Finalizadas programa': d.programCompleted, 'Cumplimiento (%)': d.compliance })) },
  ...
  async function pdf() {
    ...
    try {
      ...
      line('Día a día', 13);
      data.daily.forEach(d => line(`${shortDay(d.day)}: ${d.capturedAt ? `${d.closed} cierres del día / ${d.cumulative} acumulados / ${d.programCompleted} del programa / ${d.compliance ?? '—'}% | ${stamp(d.capturedAt)}` : 'Sin corte registrado o día futuro'}`));
      line('Las vencidas y el backlog se superponen con los estados; no sumarlos. El denominador es el programa inicial, incluidas las anuladas.');
    ...
    <FilterScopeFrame title="Avance semanal · lunes a domingo" tone="blue" hint="La semana, las zonas y el corte aplican a todos los indicadores, gráfica, detalle y exportaciones de este marco." toolbar={
      ...
      {data && <div className="space-y-5 text-slate-800 dark:text-slate-200">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <button className="rounded-xl bg-emerald-100 dark:bg-emerald-950 px-5 py-3 text-left" onClick={() => pick('completed')}><span className="block text-xs">Cumplimiento del programa inicial</span><strong className="text-3xl">{data.compliance === null ? '—' : `${data.compliance}%`}</strong></button>
        </div>
        {!data.hasData ? <p className="rounded-xl bg-amber-50 dark:bg-amber-950 p-4">No hay cortes registrados para esta semana. No se reconstruyen estados históricos a partir de las OT actuales.</p> : <>
          {(data.preview || data.plan?.lateStart) && <p className="rounded-xl bg-amber-50 dark:bg-amber-950 p-3 text-sm">{data.preview ? 'Vista previa: el programa aún no está fijado. Se fija automáticamente al comenzar la semana; las cifras pueden cambiar.' : `Seguimiento iniciado el ${stamp(data.plan?.capturedAt)}. El programa y arrastre inicial reflejan esa captura, no el lunes anterior. Los días sin corte se muestran sin datos.`}</p>}
          <p className="text-xs text-slate-500">Programa: OT con fecha límite en esta semana; si no tienen límite, se usa la fecha programada. El programa inicial y sus zonas permanecen fijos. Las anuladas se conservan en el denominador. Las vencidas usan el límite original; se superponen con los estados. Sin fecha límite no se marca vencimiento.</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {(['program', 'completed', 'pending', 'inProgress', 'paused', 'overdue', 'cancelled', 'deleted'] as WeeklyCategory[]).map(key => <button key={key} onClick={() => pick(key)} className={`rounded-xl border p-4 text-left hover:shadow-md ${category === key ? 'border-emerald-500 ring-1 ring-emerald-500' : 'border-slate-200 dark:border-slate-700'} ${key === 'overdue' ? 'bg-red-50 dark:bg-red-950/30' : 'bg-white dark:bg-slate-900'}`}><strong className="block text-2xl">{data.counts[key]}</strong><span className="text-xs">{labels[key]}</span></button>)}
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
            {(['carryover', 'incoming', 'additions', 'rescheduled', 'backlog', 'backlogOverdue', 'allCompleted'] as WeeklyCategory[]).map(key => <button key={key} onClick={() => pick(key)} className="rounded-lg border border-slate-200 dark:border-slate-700 p-3 text-left text-sm"><strong>{data.counts[key]}</strong> · {labels[key]}</button>)}
          </div>
          <p className="text-xs text-slate-500">Arrastre, trabajo nuevo y cambios son vistas complementarias y pueden compartir OT. Los cierres totales incluyen trabajo fuera del programa inicial.</p>
          <div className="grid lg:grid-cols-2 gap-5">
            <div className="h-64 min-w-0" role="img" aria-label="Gráfica de cierres acumulados totales y del programa inicial. Valores disponibles en la tabla por día.">
              <ResponsiveContainer width="100%" height="100%"><LineChart data={data.daily}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="day" tickFormatter={shortDay} fontSize={11} /><YAxis allowDecimals={false} /><Tooltip labelFormatter={value => shortDay(String(value))} /><Legend /><Line name="Cierres acumulados totales" type="monotone" dataKey="cumulative" stroke="#2563eb" connectNulls={false} /><Line name="Finalizadas del programa" type="monotone" dataKey="programCompleted" stroke="#059669" connectNulls={false} /></LineChart></ResponsiveContainer>
            </div>
            <div className="overflow-x-auto"><table className="w-full text-sm"><caption className="text-left font-semibold pb-2">Cortes por día · pulsa un día para consultar sus OT</caption><thead><tr className="text-left text-xs"><th className="p-2">Día</th><th>Cierres día</th><th>Acumulados</th><th>Del programa</th><th>Avance</th></tr></thead><tbody>{data.daily.map(d => <tr key={d.day} className="border-t border-slate-200 dark:border-slate-700"><td className="p-2"><button disabled={!d.capturedAt} className="text-blue-600 disabled:text-slate-400" title={d.capturedAt ? stamp(d.capturedAt) : 'Sin corte o día futuro'} onClick={() => { setCutId(d.cutId || ''); setCategory('allCompleted'); setDetailDay(d.day); }}>{shortDay(d.day)}</button></td><td>{d.closed ?? '—'}</td><td>{d.cumulative ?? '—'}</td><td>{d.programCompleted ?? '—'}</td><td>{d.compliance === null ? '—' : `${d.compliance}%`}</td></tr>)}</tbody></table><p className="mt-2 text-xs text-slate-500">— Sin corte registrado o día futuro. Cortes automáticos a las 23:59:59; puedes guardar uno para tu reunión. No se rellenan días omitidos.</p></div>
          </div>
