        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          <form id="create-po-form" onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Proveedor *</label>
                <select 
                  required
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Fecha Esperada</label>
                <input 
                  type="date" 
                  value={expectedDate}
                ...
                {showItemDropdown && itemSearch && (
                  <div className="absolute z-10 w-full mt-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl max-h-60 overflow-y-auto">
                    {filteredItems.length === 0 ? (
                      <div className="p-4 text-center text-sm text-slate-500 dark:text-slate-400">No se encontraron ítems</div>
                    ) : (
                      filteredItems.map(item => (
                        <div 
                          key={item.id}
                          onClick={() => handleAddItem(item)}
                          className="flex items-center gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800 dark:bg-slate-950 cursor-pointer border-b border-slate-100 dark:border-slate-800 last:border-0"
                        >
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400">{item.internal_code} • Stock actual: {item.stock}</p>
                          </div>
                          <button type="button" className="ml-auto text-emerald-600 dark:text-emerald-400 hover:text-emerald-800 dark:text-emerald-300 p-2">
                            <Plus size={18} />
                          </button>
                        </div>
                      ))
                ...
                <div className="bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                  <table className="w-full text-left text-sm">
                        ...
                        return (
                          <tr key={index} className="bg-white dark:bg-slate-900">
                            <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-100">
                              {itemData?.name || 'Item'}
                              <div className="text-xs text-slate-500 dark:text-slate-400 font-normal">{itemData?.internal_code}</div>
                            </td>
                    ...
                    <tfoot className="bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700">
                      <tr>
                        <td colSpan={3} className="px-4 py-4 text-right font-bold text-slate-600 dark:text-slate-400">Total:</td>
