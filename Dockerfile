...
# Etapa 1: compila backend (tsc) y frontend (tsc -b + vite; los tipos son bloqueantes).
...
# pg_dump/psql (respaldos/restore), tar/gzip (uploads/backups), tini (PID 1).
# El cliente PostgreSQL debe ser >= la versión del servidor (postgres:17-alpine):
# el paquete por defecto de Debian 12 es v15 y pg_dump 15 rechaza volcar PG 17.
