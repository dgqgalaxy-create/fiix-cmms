...
*(Versión 1.56.37 - 11 de Agosto, 2026)*
...
- **Mensajes (chat interno):** En el menú **Mensajes**. Puedes abrir un chat **1:1** con otro usuario activo o crear un **grupo** eligiendo título y participantes. En hilos largos usa **Cargar mensajes anteriores** arriba del chat. Adjunta imagen o archivo (PDF/docs). El badge del menú muestra no leídos; también llegan notificaciones in-app y push. En **tus** mensajes verás palomitas: una = enviado al servidor, dos grises = entregado al dispositivo del otro, dos azuladas = leído (abrió el chat). En grupo, entregado/leído cuando **todos** los demás lo cumplen. Si llega un mensaje mientras usas otra pantalla, verás un **banner arriba a la derecha**, oirás un **tono corto** y el teléfono **vibra** (Android). Con **Notificaciones del dispositivo** activadas, el aviso del sistema llega en segundo plano. En móvil, al volver a la app el chat se reconecta solo. En **interfaz móvil**, el cuadro para escribir queda encima de la barra inferior. Solo los participantes ven cada conversación. Puedes **eliminar tus mensajes** en los primeros **10 minutos**; quedan como «Mensaje eliminado».
...
- **Bitácora de auditoría:** Solo **Administrador**, en Opciones de Desarrollador. El histórico se guarda **sin límite** en la base de datos. En pantalla verás los últimos 50 eventos; puedes **descargar Excel** filtrando por fechas (Desde/Hasta, día México) o el **histórico completo**. Los imports **CSV** y **Google Sheets** quedan registrados (`IMPORT_CSV` / `IMPORT_SHEETS`, o `*_FAIL` si fallan): toca la fila para ver el detalle (conteos, fotos Drive, logos, etc.) aunque el aviso verde ya haya desaparecido.
...
- **Búsqueda global (Ctrl/Cmd+K):** Desde cualquier pantalla abre el buscador único para activos, repuestos, ubicaciones, folios FOL (p. ej. `FOL-0001`) y **zona** (p. ej. `L1`). Si la consulta coincide con una zona, aparece el atajo **Órdenes en zona…** que abre Órdenes ya filtradas. Al elegir un **repuesto**, abre su ficha en Inventario. En celular usa el icono de lupa en la barra superior.
...
- **PDF / Imprimir OC:** En el detalle de una Orden de Compra, **Descargar PDF / Imprimir** genera una hoja **Carta** vertical (21.6 × 27.9 cm, retrato) con proveedor, líneas y totales (pedido y, si está Recibida, también recibido). Solo se imprime esa hoja — no el resto de la pantalla ni páginas en blanco al final.
...
- **Imprimir / PDF:** Solo cuando el checklist ya está **enviado/firmado** o **revisado** (no en borrador). El PDF se genera en **una hoja Carta vertical** (21.6 × 27.9 cm) con la matriz completa.
...
- **Descontinuado:** En la ficha del repuesto puedes desmarcar la casilla de activo; el listado y el Excel muestran el estado como **Descontinuado** (conserva historial y movimientos; no se borra el registro).
- **Logos de proveedores:** En **Inventario → Proveedores**, cada tarjeta muestra el logo (parte superior) y los datos de contacto abajo. Al crear/editar un proveedor puedes subir una imagen; se guarda en `uploads/vendors/` y entra en el respaldo de fotos (`uploads_….tar.gz`) junto con el resto de `uploads/`.
- **Import logos de proveedores:** Misma prioridad que fotos de repuestos: **Vendors_Images.zip** (Opciones de Desarrollador → Inventario) → carpeta Drive `GOOGLE_DRIVE_VENDORS_FOLDER` (si marcas «Fotos desde Google Drive») → `data/Vendors_Images/` en el servidor. Empareja por ID (`83a52293.Logo.170018.png`). Los logos `https://…` del CSV se conservan si no hay archivo local.
...
- **Paginación (Movimientos):** El listado de movimientos pagina en el **servidor** (20 por página). Al buscar, filtrar por tipo o cambiar el orden se vuelve a la página 1. El historial de un repuesto (ficha) se carga solo para ese ítem y también pagina.
...
- **Detalle vs edición:** Al hacer clic en un repuesto se abre en **solo lectura**. Admin/Gestionador pueden editar con el botón **Editar** de la fila de acciones o el **Editar** junto a **Registrar movimiento** dentro de la ficha.
...
- **Exportar:** Excel/CSV/PDF (**Carta** horizontal, tipografía compacta). Incluyen **Falla**, **Solicitante**, **Tipo** y **Paro máquina**; mismos filtros que la tabla. En el **PDF**: columnas **Sol.** / **Ini.** / **Fin** (dd/mm/aa); **Falla / Equipo** usa el ancho restante y muestra el texto completo (sin recorte a 2 líneas).
