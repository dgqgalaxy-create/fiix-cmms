...
*(Versión 1.56.6 - 4 de Agosto, 2026)*
...
- **Mensajes (chat interno):** En el menú **Mensajes**. Puedes abrir un chat **1:1** con otro usuario activo o crear un **grupo** eligiendo título y participantes. Adjunta imagen o archivo (PDF/docs). El badge del menú muestra no leídos; también llegan notificaciones in-app y push. Si llega un mensaje mientras usas otra pantalla, verás un **banner arriba a la derecha** (aparece con zoom y se oculta solo a los 5 segundos hacia la derecha; tocarlo abre el chat), oirás un **tono corto** y el teléfono **vibra** (Android). Con **Notificaciones del dispositivo** activadas (campana), el aviso del sistema llega cuando la app está en segundo plano (con el sonido del teléfono; no se duplica con el banner si ya la estás usando). En **interfaz móvil**, el cuadro para escribir queda encima de la barra inferior. Solo los participantes ven cada conversación (el Admin no lee chats ajenos). Puedes **eliminar tus mensajes** en los primeros **10 minutos**; quedan como «Mensaje eliminado» (sin texto ni adjunto).
