...
- **Fotos de repuestos:** Antes de importar, puedes colocar las imágenes en la carpeta del proyecto `data/item-images/`, nombradas con el **código interno** del ítem (columna `Item ID` del CSV), p. ej. `MTTO-0001.jpg` o `E2-0.png` (también `.jpeg`, `.webp`, `.gif`). En Opciones de Desarrollador deja marcada la casilla **También asignar fotos desde data/item-images/**; tras crear/actualizar los ítems el sistema copia cada foto a `uploads/inventory/` (mismo formato que la subida manual) y muestra «Fotos asignadas: N». Si la carpeta está vacía, la importación CSV no se ve afectada. Detalles en `data/item-images/README.txt`.
