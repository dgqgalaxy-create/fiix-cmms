...
- **Respaldo del servidor:** El botón **Crear respaldo ahora** genera en el servidor un dump comprimido de la base de datos y una copia de `uploads/` (fotos, firmas, evidencias). El mismo proceso corre automáticamente todos los días a las 2:15 AM y conserva los respaldos de los últimos 14 días. Por defecto guarda en `~/fiix-backups` (en Windows: `%USERPROFILE%\fiix-backups`). En el servidor Ubuntu no hace falta nada extra; en Windows local necesitas las herramientas de cliente de PostgreSQL (`pg_dump`, p. ej. en `C:\Program Files\PostgreSQL\<versión>\bin`) y `tar` (incluido en Windows 10+).
