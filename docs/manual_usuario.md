...
- **Fotos de repuestos:** Antes de importar, coloca las imágenes en la carpeta del proyecto `data/Items_Images/` (casing exacto; es el nombre del export de Fiix). Los archivos reales suelen llamarse `{Item ID}.Image.{HHMMSS}.{ext}`, p. ej. `MTTO-0001.Image.163526.png` (coincide con la columna `Image` del CSV `Items - Items.csv` y con el `Item ID` / `internal_code`). Extensiones: `.jpg`, `.jpeg`, `.png`, `.webp`, `.gif`. En Opciones de Desarrollador deja marcada la casilla **También asignar fotos desde data/Items_Images/**; tras crear/actualizar los ítems el sistema escanea esa carpeta, extrae el Item ID del nombre, copia cada foto a `uploads/inventory/` (mismo formato que la subida manual) y muestra «Fotos asignadas: N». Si la carpeta está vacía, la importación CSV no se ve afectada. Detalles en `data/Items_Images/README.txt`.
