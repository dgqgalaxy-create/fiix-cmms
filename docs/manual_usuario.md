...
- **Calendario (pendientes):** la lista lateral de OT pendientes sin programar requiere el permiso **Administrar Calendario**. En **escritorio** arrastra con el ratón; en **tablet/celular** arrastra con el dedo y suelta sobre el día (o franja horaria) para asignar. El módulo usa la misma URL del API que el resto del sistema (host del servidor), no `localhost`.
