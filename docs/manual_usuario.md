...
- **Exportar:** botón **Excel** (.xlsx real, lista filtrada), **CSV** y **PDF**/impresión (**Carta** horizontal 21.6 × 27.9 cm: folio, **falla**/equipo, fecha y estado; ~10–15 órdenes por hoja). Excel/CSV/PDF usan la **descripción de la falla** (texto completo de la solicitud), no el título corto; si no hay descripción, se usa el título. El PDF aplica los mismos filtros que Excel/CSV (incluida la búsqueda por solicitante).
