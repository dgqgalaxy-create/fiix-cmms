#!/usr/bin/env bash
# Prueba de integración AISLADA: cierre de OT + consumo de repuestos (todo-o-nada).
# Crea una base temporal fiix_cmms_test_*, aplica migraciones, corre las pruebas
# con ts-node y elimina la base al terminar. NUNCA toca la base operativa (fiix_cmms).
set -euo pipefail
...
TEST_DB="fiix_cmms_test_atomic_close"
...
echo ">> Ejecutando pruebas de cierre atómico…"
DATABASE_URL="$TEST_URL" npx ts-node --transpile-only -P tests/tsconfig.json tests/atomicClose.integration.ts
