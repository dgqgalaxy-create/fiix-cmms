#!/usr/bin/env bash
# Prueba de integración AISLADA: borrado de notificaciones.
...
TEST_DB="fiix_cmms_test_notifdel"
...
echo ">> Ejecutando pruebas del borrado de notificaciones…"
DATABASE_URL="$TEST_URL" npx ts-node --transpile-only -P tests/tsconfig.json tests/notificationsDelete.integration.ts
