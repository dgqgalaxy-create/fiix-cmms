#!/usr/bin/env bash
# Prueba de integración AISLADA: protección de stock y permisos de compras.
...
TEST_DB="fiix_cmms_test_stock_purchase"
...
echo ">> Ejecutando pruebas de stock y compras…"
DATABASE_URL="$TEST_URL" npx ts-node --transpile-only -P tests/tsconfig.json tests/stockProtection.integration.ts
