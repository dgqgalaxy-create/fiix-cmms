#!/usr/bin/env bash
# Prueba de integración AISLADA: expediente anual.
...
echo ">> Ejecutando pruebas del expediente anual…"
DATABASE_URL="$TEST_URL" npx ts-node --transpile-only -P tests/tsconfig.json tests/annualFile.integration.ts
