#!/usr/bin/env bash
# Prueba de integración AISLADA: auditoría con antes/después.
...
TEST_DB="fiix_cmms_test_auditchanges"
...
echo ">> Ejecutando pruebas del auditoría con antes/después…"
DATABASE_URL="$TEST_URL" npx ts-node --transpile-only -P tests/tsconfig.json tests/auditChanges.integration.ts
