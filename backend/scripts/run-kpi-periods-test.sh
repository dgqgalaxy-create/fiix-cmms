#!/usr/bin/env bash
# Prueba de integración AISLADA: KPIs (disponibilidad y periodos en hora de planta).
...
echo ">> Ejecutando pruebas de KPIs…"
DATABASE_URL="$TEST_URL" npx ts-node --transpile-only -P tests/tsconfig.json tests/kpiPeriods.integration.ts
