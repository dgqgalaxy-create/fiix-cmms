#!/usr/bin/env bash
# Prueba de integración AISLADA: endurecimientos (lock, SSRF, preferencias).
...
TEST_DB="fiix_cmms_test_hardening"
...
echo ">> Ejecutando pruebas del endurecimientos (lock, SSRF, preferencias)…"
DATABASE_URL="$TEST_URL" npx ts-node --transpile-only -P tests/tsconfig.json tests/hardening.integration.ts
